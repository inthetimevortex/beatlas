...
def corner(
    ...
    **hist2d_kwargs
    ...
    color : str
        A ``matplotlib`` style color for the quantiles lines. (MODIFIED BY AMANDA)
    ...
    color_hist : str
        Color used for the histograms (ADDED BY AMANDA <3)

    color_dens : str
        Color used for the density plots (ADDED BY AMANDA <3)
    ...
    **hist2d_kwargs
    ...
    if hist_kwargs is None:
        hist_kwargs = dict()
    hist_kwargs["color"] = hist_kwargs.get("color", color_hist)
    if smooth1d is None:
        ...
        if show_titles:
            ...
            if title_fmt is not None:
                ...
                if labels is not None:
                    title = "{0} = {1}".format(labels2[i], title)
...
def hist2d(
    ...
    **kwargs
    ...
    density_cmap = LinearSegmentedColormap.from_list(
        "density_cmap", [color_dens, (1, 1, 1, 0)]
    )
    ...
    # contours are filled.
    rgba_color = colorConverter.to_rgba(color_dens)
